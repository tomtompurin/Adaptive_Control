...
t=0:dt:50;
r=zeros(1,length(t));
% d=(1+t).^(-1/5).*(5-(1+t).^(-1/5)-0.4*(1+t).^(-6/5)); % 図4.3の場合の外乱
d=zeros(1,length(t)); % 図4.4の場合の外乱
...
for ii=1:length(t)
    ...
    K1=dt*(-sigma*theta(ii)+g1*y(ii)*e(ii));
    K2=dt*(-sigma*(theta(ii)+0.5*K1)+g1*y(ii)*e(ii));
    K3=dt*(-sigma*(theta(ii)+0.5*K2)+g1*y(ii)*e(ii));
    K4=dt*(-sigma*(theta(ii)+K3)+g1*y(ii)*e(ii));
    theta(ii+1)=theta(ii)+(K1+2*K2+2*K3+K4)/6;
end
%% figure
figure('Name','定数型σ修正法の場合の応答')
